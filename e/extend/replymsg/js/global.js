	...
	function chkReply(id){
		...
		var from_username=$('#from_username'+id).val();
		var to_username=$('#to_username'+id).val();
		var nowtime=new Date().toLocaleString();
		if(rc==''){ 
			alert('回复内容不能为空');
			return;
		}
		var post_data="action=replyMsg&Id="+id+"&from_username="+from_username+"&to_username="+to_username+"&replyContent="+rc;
		//alert(post_data);
		if(from_username|=''){ 
			alert('回复人不能为空');
			return;
