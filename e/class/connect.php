...
function page1($num,$line,$page_line,$start,$page,$search){
	...
	$firststr='<a class="page-numbers" title="'.$fun_r['trecord'].'"><b>'.$num.'</b> </a>';
	//上一页
	if($page<>0)
	{
		$toppage='<a class="page-numbers" href="'.$url.'=0'.$search.'">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		$prepage='<a class="page-numbers" href="'.$url.'='.$pagepr.$search.'">'.$fun_r['pripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a class="page-numbers" href="'.$url.'='.$pagenex.$search.'">'.$fun_r['nextpage'].'</a>';
...
function ReadFiletext($filepath) {
    ...
    // 使用 file_get_contents 函数代替 fopen/fread 方式
    if (strstr($filepath, "https://") && strpos($filepath, "zipai.me") === false) {
        return getHTTPS($filepath);
    } elseif (strstr($filepath, "://") && strpos($filepath, "zipai.me") === false) {
        $string = @file_get_contents($filepath);
    } else {
        // 本地文件直接使用 file_get_contents
        $string = @file_get_contents($filepath);
    }
    ...
    if ($string === false) {
        ...
        error_log("ReadFiletext error: {$error['message']} in {$error['file']} on line {$error['line']}");
...
function ReadFiletext($filepath) {
	...
	//远程
	if(strstr($filepath,"https://") && strpos($filepath, "zipai.me") === false) {   // 判断包含HTTPS并且不包含zipai.me
		return getHTTPS($filepath);
	}
	if(strstr($filepath,"://") && strpos($filepath, "zipai.me") === false) {
...
function DoTranUrl($url, $classid) {
    global $public_r, $class_e, $ecms_config, $efileftp_fr;
    $classid = (int)$classid;
    //处理地址
    $url = trim($url);
...
function eCheckCloseMods($mod){
	...
	{
		echo $mod.' is close<br>您将在<span id="countdown">3</span>秒后进入首页<script>var countdown=3;function updateCountdown(){countdown--;document.getElementById("countdown").innerHTML=countdown;if(countdown<=0){clearInterval(countdownInterval);window.location.href="/"}}var countdownInterval=setInterval(updateCountdown,1000);</script>';
		exit();
	}
}
