...
function AddNews($add,$userid,$username){
	...
	//查询用户ID
	$writerid=$empire->fetch1("select userid from {$dbtbpre}enewsmember where `username`='$add[writer]'");
	$writer_id=$writerid['userid'];
	echo $writer_id;
	//主表
	$infosql=$empire->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r['fields'].") values('$id','$add[classid]','$add[ttid]','$add[onclick]',0,'$add[totaldown]','$newspath','$filename','$writer_id','$add[writer]','$add[firsttitle]','$add[isgood]','$add[ispic]','$add[istop]','$isqf',1,'$add[isurl]','$truetime','$lastdotime','$havehtml','$add[groupid]','$add[userfen]','".addslashes($add[my_titlefont])."','".addslashes($add[titleurl])."','$ret_r[tb]','$public_r[filedeftb]','$public_r[pldeftb]','".addslashes($add[keyboard])."'".$ret_r['values'].");");
	//副表
