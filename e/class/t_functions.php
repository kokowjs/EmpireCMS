...
function sys_ShowListPage($num,$pagenum,$dolink,$dotype,$page,$lencord,$ok,$search="",$add){
	...
	$pagelink=$pagetop.$pagepri.$pagenext;
	//替换模板变量
...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1="<span class='page-numbers current'><span class='meta-nav screen-reader-text'>第 </span>";
			$is_2="<span class='meta-nav screen-reader-text'> 页</span></span>";
		}
		elseif($i==1)
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add['dofile'].$type.'"><span class="meta-nav screen-reader-text">第 </span>';
			$is_2='<span class="meta-nav screen-reader-text"> 页</span></a>';
		}
		else
		{
			$thispagelink=$repagenum&&$repagenum<$i?eReturnRewritePageLink2($add,$i):$dolink.$add['dofile'].'_'.$i.$type;
			$is_1='<a class="page-numbers" href="'.$thispagelink.'"><span class="meta-nav screen-reader-text">第 </span>';
			$is_2='<span class="meta-nav screen-reader-text"> 页</span></a>';
		}
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add[filename].$type.'"><span class="meta-nav screen-reader-text">第 </span>';
			$is_2='<span class="meta-nav screen-reader-text"> 页</span></a>';
		}
		else
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add[filename].'_'.$i.$type.'"><span class="meta-nav screen-reader-text">第 </span>';
			$is_2='<span class="meta-nav screen-reader-text"> 页</span></a>';
		}
		$returnstr.='&nbsp;'.$is_1.$i.$is_2;
	}
...
function sys_ResizeImg($file,$width,$height,$docut=0,$target_filename='',$target_path='e/data/tmp/titlepic/'){
	...
	if(!$file||!$width||!$height)   // 如果缺少必要的参数，返回原始文件
	{
		return $file;
	}
	//扩展名
	$filetype=GetFiletype($file);   // 获取文件的扩展名
	if(!strstr($ecms_config['sets']['tranpicturetype'],','.$filetype.','))  // 检查文件类型是否允许调整大小
	{
		return $file;
	}
	$efileurl=eReturnFileUrl();     // 获取网站文件URL的基础路径
	//20231030更新
	$dfileurl = $efileurl.'selfies/'.date("Ym");
    if (strstr($file,$efileurl)) {
        $file=str_replace($efileurl,'/d/file/',$file);
    }
    //20231030更新
	if(strstr($file,'://'))     // 如果文件路径包含 '://'，则返回原始文件
	{
		return $file;
	}
	$filename=eReturnEcmsMainPortPath().substr($file,1);//moreport 构建文件的完整路径
	if(!file_exists($filename))     // 如果文件不存在，返回原始文件
	{
		return $file;
	}
	if($target_filename)        // 设置新文件的名称
	{
	...
	$newpath=ECMS_PATH.$target_path;        // 设置新文件的路径和URL
	$newurl=$public_r['newsurl'].$target_path;
	$newname=$newpath.$newfilename;
	//if(empty($target_filename)&&file_exists($newname.$filetype))
	if(empty($target_filename)&&is_file($newname.$filetype))    // 如果未指定目标文件名且新文件已存在，返回新图像的URL
	{
		return $newurl.$newfilename.$filetype;
	}
	if(!defined('InEmpireCMSGd'))   // 如果未定义 'InEmpireCMSGd'，包含GD库文件
	{
		include_once ECMS_PATH.'e/class/gd.php';
	}
	$filer=ResizeImage($filename,$newname,$width,$height,$docut);   // 调整图像大小并返回新文件的URL
	$fileurl=$newurl.$newfilename.$filer['filetype'];
	return $fileurl;    // 返回新生成图像的URL
}
