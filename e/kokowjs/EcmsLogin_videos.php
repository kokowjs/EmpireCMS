...
if(!$_POST['videourl']) exit(mb_convert_encoding('视频地址为空:', "UTF-8", "GBK").$loginin);
...
if(!$writer_id)//添加会员信息
{
	$empire->fetch1("INSERT INTO `{$dbtbpre}enewsmember` (userid,username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey) VALUES ('', '$writer', 'f0e9b41d5a7b56cf7205090cd8a02bc1', 'BQYKThyDvNr6WRYmxBUh', 'yinsibaohu@5zipai.com', '978282000', '8', '0', '0', '0.00', '0', '0', '1', 't7fPZN', 'ADSG8zKwMEOl')");

	$writer_id = $empire->lastid();

	$enewsmemberadd=$empire->fetch1("INSERT INTO `{$dbtbpre}enewsmemberadd` (userid,truename,oicq,msn,mycall,phone,address,zip,spacestyleid,homepage,saytext,company,fax,userpic,spacename,spacegg,viewstats,regip,lasttime,lastip,loginnum,regipport,lastipport) VALUES ('$writer_id', '$writer', '', '', '', '', '', '', '1', '//www.5zipai.com/myvideo/$writer_id/', '', '', '', '', '', '', '0', '8.8.8.8', '978282000', '8.8.8.8', '0', '', '')");
}
//验证重复
/*
$m_title=$_POST['title'];
$m_lur=$empire->fetch1("select * from {$dbtbpre}ecms_videos where `title`='$m_title'");
