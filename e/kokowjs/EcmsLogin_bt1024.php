...
if(!$_POST['newstext']) exit(mb_convert_encoding('内容为空:', "UTF-8", "GBK").$loginin);
...
if(!$writer_id)//添加会员信息
{
	$empire->fetch1("INSERT INTO `{$dbtbpre}enewsmember` (userid,username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey) VALUES ('', '$writer', 'f0e9b41d5a7b56cf7205090cd8a02bc1', 'BQYKThyDvNr6WRYmxBUh', 'yinsibaohu@yuanfou.net', '978282000', '3', '0', '0', '0.00', '0', '0', '1', 't7fPZN', 'ADSG8zKwMEOl')");
...
$m_title=$_POST['title'];
$m_lur=$empire->fetch1("select * from {$dbtbpre}ecms_news where `title` like '%$m_title%'");
