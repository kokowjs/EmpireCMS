...
// 设置缓存键名
$cacheKey = 'article_' . $id . '_' . $down;

// 尝试从缓存中获取数据
$shownum = $memcached->get($cacheKey);
...
if (!$shownum) {
    ...
    if ($down == 0) {
        ...
        $shownum = $r['onclick'] + 1;
        if ($_GET['addclick'] == 1) {
            $usql = $empire->updatesql("update {$dbtbpre}ecms_".$cr['tbname']." set onclick=onclick+1 where id='$id' limit 1");
        }
    }
    // 下载数
    elseif ($down == 1) {
    ...
    // 将数据存入缓存中，缓存时间为一个小时
    $memcached->set($cacheKey, $shownum, 3600);
}
